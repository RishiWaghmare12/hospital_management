...
const Appointments = () => {
  ...
  */
    ...
    const fetchAppointments = async () => {
      try {
        ...
        if (response.data) {
          ...
          await processAppointments(fetchedAppointments);
        }
      } catch (err) {
    ...
    for (const appt of appointments) {
      try {
        ...
        let patientName = appt.patientName ||
          (appt.patient ? appt.patient.name : null);
        ...
        // If we don't have a patient name but have an ID, fetch it
        if (!patientName && patientId !== 'N/A') {
          try {
            const patientResponse = await patientService.getPatientById(patientId);
            if (patientResponse.data) {
              patientName = patientResponse.data.name || patientResponse.data.patientName || `Patient ${patientId}`;
            }
          } catch (error) {
        ...
        if (!patientName) {
          patientName = patientId !== 'N/A' ? `Patient ${patientId}` : 'Patient';
        }

        // Log patient ID extraction for debugging
        console.log('Processing appointment - extracted patient ID:', patientId, 'name:', patientName);
  ...
  const openPrescriptionModal = () => {
    console.log('[Appointments] openPrescriptionModal called with selected:', selectedAppointment);
    setShowSimplePrescriptionForm(true);
  };

  // Open prescription form directly for a given appointment from the list
  const openPrescriptionFor = (appointment) => {
    console.log('[Appointments] Opening prescription for appt:', appointment);
    setSelectedAppointment(appointment);
    setShowSimplePrescriptionForm(true);
  };
  ...
  return (
    <div className="space-y-8 p-6">
      ...
      <div className="bg-gradient-to-r from-emerald-400 via-teal-500 to-cyan-600 rounded-xl shadow-xl mb-8 text-white p-6 flex justify-between items-center hover:shadow-2xl transition-shadow duration-300">
        <div>
          <h1 className="text-3xl font-bold flex items-center">
            <CalendarIcon className="h-8 w-8 mr-2 text-white" /> Appointments
          </h1>
          <p className="mt-1 text-emerald-100">Manage your patient appointments</p>
        </div>
      </div>
      ...
      <div className="mb-6 flex space-x-4">
        {['today', 'upcoming', 'past'].map((tab) => (
          <button
            ...
            className={`px-6 py-3 rounded-full font-medium text-sm capitalize transition-all duration-300 shadow-sm border-2 ${selectedTab === tab
              ? 'bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-200 transform scale-105 border-emerald-600'
              : 'bg-white text-gray-600 hover:bg-emerald-50 hover:text-emerald-600 border-gray-200'
              }`}
          >
            {tab} <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${selectedTab === tab ? 'bg-white/20 text-white border border-white/30' : 'bg-gray-100 text-gray-600 border border-gray-200'}`}>{filteredAppointments[tab].length}</span>
          </button>
        ))}
      </div>
      ...
      <div className="space-y-6">
        {loading ? (
          <div className="p-12 text-center bg-white rounded-2xl shadow-lg border border-gray-100">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-emerald-500 mx-auto"></div>
            <p className="mt-4 text-gray-600 font-medium">Loading appointments...</p>
          </div>
        ) : error ? (
          <div className="p-8 text-center text-red-500 bg-white rounded-2xl shadow-lg border border-red-100">
            <ExclamationCircleIcon className="h-12 w-12 mx-auto mb-4 text-red-400" />
            <p className="font-medium text-lg">{error}</p>
            <button
              onClick={() => window.location.reload()}
              className="mt-4 px-6 py-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors shadow-md"
            >
              Retry
            </button>
          </div>
        ) : filteredAppointments[selectedTab].length === 0 ? (
          <div className="p-16 text-center bg-white rounded-3xl shadow-lg border-2 border-emerald-400 border-l-[8px] border-l-emerald-500">
            <div className="w-24 h-24 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner">
              <CalendarDaysIcon className="h-12 w-12 text-emerald-400" />
            </div>
            <h3 className="text-2xl font-bold text-gray-900 mb-3">No {selectedTab} appointments found</h3>
            <p className="text-gray-500 max-w-md mx-auto text-lg">
              {selectedTab === 'today'
                ? 'You have no appointments scheduled for today. Enjoy your day!'
                : selectedTab === 'upcoming'
                  ? 'You have no upcoming appointments scheduled at the moment.'
                  : 'No past appointment records found in your history.'}
            </p>
          </div>
        ) : (
          filteredAppointments[selectedTab].map((appointment) => (
            <div key={appointment.id} className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col md:flex-row md:items-center md:justify-between p-6 gap-6 border border-gray-100 hover:shadow-2xl transition-shadow">
              ...
              <div className="flex flex-col gap-2 mt-4 md:mt-0 md:ml-6">
                ...
                {/* Allow creating a prescription directly from the card */}
                <button
                  onClick={() => openPrescriptionFor(appointment)}
                  className="px-5 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors font-semibold shadow"
                >
                  Create Prescription
                </button>
              </div>
