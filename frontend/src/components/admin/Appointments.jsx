...
const Appointments = () => {
    ...
    try {
      ...
      if (response && response.data) {
        ...
        const normalizedAppointments = response.data.map(appointment => {
          // Use pId as the primary patient ID field (matching backend)
          const patientId = appointment.pId ||
            appointment.patientId ||
            appointment.P_ID ||
            appointment.patient_id ||
            (appointment.patient ? (appointment.patient.pId || appointment.patient.pid || appointment.patient.id) : null);

          // Return normalized appointment with consistent pId field
          return {
            ...appointment,
            pId: patientId,
  ...
  const fetchPatients = async () => {
    try {
      ...
      if (response && response.data) {
        ...
        // Extract and transform patients for consistent structure
        const normalizedPatients = response.data.map(patient => {
          // Use pId as the primary field (matching backend Patient entity)
          const pId = patient.pId || patient.pid || patient.P_ID || patient.id || patient.patientId;
          return {
            ...patient,
            pId: pId,
            pid: pId  // Keep for backward compatibility
          };
        });
  ...
  const fetchPatientById = async (patientId) => {
    ...
    try {
      ...
      if (patientData) {
        ...
        const pId = patientData.pId || patientData.pid || patientData.P_ID || patientData.id || patientData.patientId || patientId;
        const patient = {
          ...patientData,
          pId: pId,
          pid: pId,
  ...
  const getPatientProperty = (appointment, prop) => {
    ...
    // For ID property, return the pId
    if (prop === 'id') {
      const id = appointment.pId || appointment.patientId || '';
    ...
    if (prop === 'name') {
      // Get the patient ID from the appointment (use pId as primary)
      const patientId = appointment.pId || appointment.patientId;
      ...
      console.log(`Getting name for patient ID: ${patientId}`);
      ...
      if (patients.length > 0) {
        ...
        if (patient) {
          ...
          if (patientName) {
            console.log(`Found patient name: ${patientName} for ID: ${patientId}`);
            return patientName;
          }
        } else {
          // Patient not found in array, try to fetch it
          console.log(`Patient ID ${patientId} not found in array, attempting to fetch`);
  ...
  return (
    <div className="space-y-8 p-6">
      {/* Vibrant Header */}
      <div className="bg-gradient-to-r from-orange-400 via-orange-500 to-orange-600 rounded-xl shadow-xl mb-8 text-white p-6 flex justify-between items-center hover:shadow-2xl transition-shadow duration-300">
        <div>
          <h1 className="text-3xl font-bold flex items-center">
            <CalendarIcon className="h-8 w-8 mr-2 text-white" /> Appointments Management
          </h1>
          <p className="mt-1 text-orange-100">Manage and track all patient appointments</p>
        </div>
      </div>

      {/* Search and Filters */}
      <div className="bg-gradient-to-br from-orange-50 via-orange-50 to-white rounded-2xl shadow-lg p-6 border-2 border-orange-400 border-l-[8px] border-l-orange-500 mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all"
              value={searchQuery}
          ...
          <div className="relative min-w-[150px]">
            ...
            <select
              className="pl-10 pr-4 py-3 bg-white border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 w-full appearance-none transition-all"
      ...
      {/* Appointments Table */}
      <div className="bg-gradient-to-br from-orange-50 via-orange-50 to-white rounded-2xl shadow-lg border-2 border-orange-400 overflow-hidden">
        {loading && !appointments.length ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gradient-to-r from-orange-100 via-orange-100 to-orange-100 border-b-2 border-orange-300">
                <tr>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {getFilteredAppointments().map((appointment) => (
                  <tr key={appointment.id || appointment.apId} className="hover:bg-orange-100 hover:scale-[1.001] transition-all duration-200">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-8 w-8">
                          <div className="h-8 w-8 rounded-full bg-gradient-to-br from-rose-100 to-pink-100 flex items-center justify-center">
                            <UserIcon className="h-5 w-5 text-rose-600" />
                          </div>
                        </div>
                        <div className="ml-3">
                          <div className="text-sm font-medium text-gray-900">
                            {getPatientProperty(appointment, 'name')}
                          </div>
                          <div className="text-sm text-gray-500">
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <div className="mt-3 text-gray-600">
                ...
                <div className="mt-2 p-3 bg-gray-50 rounded-lg">
                  <p className="text-sm font-medium">Patient: {getPatientProperty(appointmentToDelete, 'name')}</p>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {patients.map(patient => (
                        <option
                          key={patient.pId || patient.pid}
                          value={patient.pId || patient.pid}
                        >
            ...
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg border-l-4 border-green-500">
              ...
              <div className="bg-green-50 p-3 rounded-lg mb-5">
                <p className="font-medium text-gray-800">
                  <span className="font-bold">Patient:</span> {getPatientProperty(selectedAppointment, 'name')}
                </p>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const prescriptionData = {
                        ...
                        P_ID: selectedAppointment.pId || selectedAppointment.patientId,
                        medicine: medicine,
                        advice: advice,
                        remark: remark
                      };
