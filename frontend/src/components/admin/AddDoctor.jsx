...
const AddDoctor = ({ onClose, onDoctorAdded }) => {
  ...
  useEffect(() => {
    const fetchSpecialties = async () => {
      try {
        // Fetch actual specialties from backend; avoid client-side defaults
        const res = await specialtyService.getAllSpecialties();
        const data = Array.isArray(res?.data) ? res.data : [];
        setSpecialties(data);
        if (!Array.isArray(res?.data) || res.data.length === 0) {
          setSpecialtyError('No specialties available. Please create a specialization first.');
          toast.warn('No specialties available. Please add a specialization first.');
        }
      } catch (error) {
        console.error('Error fetching specialties:', error);
        setSpecialtyError('Failed to load specialties from server.');
        toast.error('Failed to load specialties from server.');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (specialties.length === 0) {
        throw new Error('No specialties configured. Please create a specialization first.');
      }
      ...
      // Prepare payload with numeric fields coerced
      const payload = {
        ...formData,
        age: Number(formData.age),
        experience: Number(formData.experience),
        spId: Number(formData.spId)
      };

      // Send doctor data as JSON
      const response = await doctorService.addDoctor(payload);
            ...
            <div>
              ...
              >
                ...
                {isLoadingSpecialties ? (
                  ...
                  <option value="" disabled>{specialtyError || 'No specialties found'}</option>
                ) : (
